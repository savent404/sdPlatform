...
static inline void
dlist_add(dlist_t _prev, dlist_t _next, dlist_t _new)
{
    _new->next = _next;
    _new->prev = _prev;
    _next->prev = _new;
...
static inline void
dlist_del(dlist_t _prev, dlist_t _next, dlist_t _node)
{
...
static inline void
dlist_add_back(dlist_t _node, dlist_t _new)
{
    dlist_add(_node, _node->next, _new);
}

static inline void
dlist_add_front(dlist_t _node, dlist_t _new)
{
    dlist_add(_node->prev, _node, _new);
}

static inline void
dlist_remove(dlist_t _node)
{
    dlist_del(_node->prev, _node->next, _node);
}

static inline dlist_t
dlist_next(dlist_t _node)
{
    return _node->next;
}

static inline dlist_t
dlist_prev(dlist_t _node)
{
    return _node->prev;
}

static inline int
dlist_isalone(dlist_t _node)
{
    return _node->next == _node;
}

static inline bool
dlist_has_item(dlist_t _head, dlist_t _node)
{
    dlist_t item = _head;
    do {
        item = _head->next;
